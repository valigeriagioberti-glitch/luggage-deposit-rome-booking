...
const SuccessView: React.FC<SuccessViewProps> = ({ booking, onReset }) => {
  if (!booking) {
    // Extra safety: should not happen because SuccessPage guards,
    // but we keep this to prevent blank screens in any case.
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white rounded-xl border border-gray-100 shadow-sm p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">We couldn’t load the booking details.</p>
          <button
            onClick={onReset}
            className="bg-green-900 text-white py-3 px-6 rounded-lg font-bold hover:bg-green-800 transition"
          >
            New Booking
          </button>
        </div>
  ...
  // NOTE: Your current "Download PDF" uses print too.
  // If you want real PDF download, we can add it later using a backend or a PDF library.
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden print:hidden">
        <div className="bg-green-50 p-8 text-center border-b border-gray-100">
          ...
          <p className="text-gray-600 max-w-md mx-auto">
            Your luggage storage reservation is confirmed. We've sent a confirmation email to{' '}
              ...
              <div className="space-y-3 pl-2 border-l-2 border-gray-100 ml-2">
                ...
                <div>
                  ...
                  <p className="font-medium text-green-900">
                    {booking.billableDays} Day{booking.billableDays > 1 ? 's' : ''}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4 text-sm space-y-2 border border-gray-100">
                ...
                <p>
                  ...
                  <span className="font-medium text-gray-900">{new Date(booking.timestamp).toLocaleString()}</span>
                </p>
              </div>
            </div>
          </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  ...
                  <tr className="bg-gray-50">
                    <td colSpan={3} className="px-6 py-4 text-sm font-bold text-gray-700 text-right">
                      Per-Day Subtotal
                    </td>
                    <td className="px-6 py-4 text-sm font-bold text-gray-900 text-right">
                      €{dailySubtotal.toFixed(2)}
                    </td>
                  </tr>
                </tbody>
              </table>
